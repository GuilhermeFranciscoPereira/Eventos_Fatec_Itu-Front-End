...
export default function Login(): React.ReactElement {
    ...
    const { showPasswordRef, svgContainerRef, showPassword } = useYeti({ emailRef, passwordRef });
        ...
        <main className={styles.mainLogin}>
            <form className={styles.form} onSubmit={handleSubmit}>
                <div className={styles.inputGroup}>
                    ...
                    {errors.email && (<span className={styles.errorMessage}><TbAlertTriangleFilled /> {errors.email}</span>)}
                </div>

                <div className={styles.inputGroup}>
                    <InputDefault ref={passwordRef} id="loginPassword" label="password" type="password" title="Senha" />
                    {errors.password && (<span className={styles.errorMessage}> <TbAlertTriangleFilled /> {errors.password} </span>)}
                    <label htmlFor="showPasswordCheck" className={styles.showPasswordToggle}>
                        <span className={styles.iconEye}>{showPassword ? <FaRegEye /> : <LuEyeOff />}</span>
                        <input ref={showPasswordRef} id="showPasswordCheck" type="checkbox" className={styles.showPasswordCheck} />
                    </label>
                </div>

                <div>
                    {loading && <Loader />}
                    <ButtonRay text={'Acessar'} disable={loading} type="submit" />
                </div>

                <Link href={'/Login/ResetPassword'} className={styles.forgotPassword}>
                    <button type="button" className={styles.forgotButton}>Esqueci minha senha</button>
                </Link>
            </form>

            {/* YETI ICON - JUST IGNORE! Message By: Guilherme Francisco Pereira */}

            <div ref={svgContainerRef} className={styles.svgContainer}>
                <div className={styles.svgContainerWrapper}>
                    <svg className={styles.svgContainerWrapperMySVG} xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 200 200">
                        ...
                        <g className="earL">
                            <g className="outerEar" fill="#ddf1fa" stroke="#3a5e77" strokeWidth="2.5"><circle cx="47" cy="83" r="11.5" />
                        ...
                        <g className="mouth">
                            ...
                            <g clipPath="url(#mouthMask)">
                                <g className="tongue"> <circle cx="100" cy="107" r="8" fill="#cc4a6c" /> <ellipse className="tongueHighlight" cx="100" cy="100.5" rx="3" ry="1.5" opacity=".1" fill="#fff" /></g>
                            </g>
                        ...
                        <g className="arms" clipPath="url(#armMask)">
                            <g className="armL" style={{ visibility: "hidden" }}>
                                ...
                                <g className="twoFingers"> <path fill="#DDF1FA" stroke="#3A5E77" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" strokeMiterlimit="10" d="M158.3,67.8l23.1-6.2c2.7-0.7,5.4,0.9,6.1,3.5v0c0.7,2.7-0.9,5.4-3.5,6.1l-23.1,6.2" /> <path fill="#A9DDF3" d="M180.1,65l2.2-0.6c1.1-0.3,2.2,0.3,2.4,1.4v0c0.3,1.1-0.3,2.2-1.4,2.4l-2.2,0.6L180.1,65z" /> <path fill="#DDF1FA" stroke="#3A5E77" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" strokeMiterlimit="10" d="M160.8,77.5l19.4-5.2c2.7-0.7,5.4,0.9,6.1,3.5v0c0.7,2.7-0.9,5.4-3.5,6.1l-18.3,4.9" /> <path fill="#A9DDF3" d="M178.8,75.7l2.2-0.6c1.1-0.3,2.2,0.3,2.4,1.4v0c0.3,1.1-0.3,2.2-1.4,2.4l-2.2,0.6L178.8,75.7z" />
                                </g>
